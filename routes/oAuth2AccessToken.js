...
function IsRequestFromChatBot(req){
    if (req.body.payload.hasOwnProperty('actionItem') || req.body.payload.hasOwnProperty('cmd')) {
        return true
    }
    return false
}
...
var getAccessToken = function (req, res, next) {
    ...
    if (req.headers.authorization === process.env.zoom_verification_token && IsRequestFromChatBot(req)) {
        ...
        pg.query(`SELECT * FROM oauth_tokens WHERE user_id != '' LIMIT 1`, (error, results) => {
          if (error) {
            console.log('Error getting oauth2 access_token from database.', error)
          } else {
            if (results.rows[0].expires_on > (new Date().getTime() / 1000)) {
              ...
              getFreshAccessTokenFromZoom(req, res, next, results.rows[0].refresh_token, results.rows[0].user_id)
            }
          }
        })
    } else {
...
function getFreshAccessTokenFromZoom (req, res, next, refresh_token, user_id) {
    request({
      ...
      if (error || JSON.parse(body).error) {
        ...
        pg.query(`UPDATE oauth_tokens SET access_token = '${body.access_token}', refresh_token = '${body.refresh_token}', expires_on = '${(new Date().getTime() / 1000) + body.expires_in - 60}' WHERE user_id = '${user_id}'`, (error, results) => {
          if (error) {
            console.log('Error setting oauth2 access_token in database. ', error)
          } else {
